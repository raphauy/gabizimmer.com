...
export async function updateUserRoleAction(userId: string, role: string) {
  try {
    ...
    await updateUser(userId, { 
      role: role as "superadmin" | "colaborador" | null
    })
...
export async function createUserAction(formData: FormData) {
  try {
    ...
    await createUser({
      email,
      name: name || undefined,
      role: role as "superadmin" | "colaborador"
    })

    revalidatePath("/admin/users")
    return { success: true, message: "Colaborador invitado correctamente" }
  } catch (error) {
    console.error("Error creando usuario:", error)
    return { 
      success: false, 
      error: error instanceof Error ? error.message : "Error invitando colaborador" 
    }
  }
}